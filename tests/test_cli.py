...
def test_four_digit_year_only(capsys: pytest.CaptureFixture[str]) -> None:
...
def test_two_digit_year_expansion(capsys: pytest.CaptureFixture[str]) -> None:
    with freeze(Date(2025, 6, 1)):
        # 22 <= 25, so 22 -> 2022
...
def test_two_digit_year_falls_back_to_1900s(capsys: pytest.CaptureFixture[str]) -> None:
    with freeze(Date(2025, 6, 1)):
        # 94 > 25, so 94 -> 1994
...
def test_birthday_today_prints_turned_message(capsys: pytest.CaptureFixture[str]) -> None:
    with freeze(Date(2025, 6, 1)):
...
def test_future_date_reports_years_in_future(capsys: pytest.CaptureFixture[str]) -> None:
    with freeze(Date(2025, 6, 1)):
...
def test_invalid_year_length_exits(capsys: pytest.CaptureFixture[str]) -> None:
    with freeze(Date(2025, 6, 1)), pytest.raises(SystemExit) as excinfo:
        main(["123"])
...
def test_invalid_calendar_date_exits(capsys: pytest.CaptureFixture[str]) -> None:
    with freeze(Date(2025, 6, 1)), pytest.raises(SystemExit) as excinfo:
        main(["2024", "02", "30"])
    assert excinfo.value.code == 2
    err = capsys.readouterr().err
